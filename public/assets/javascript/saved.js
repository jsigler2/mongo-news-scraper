$(function() {
    ...
    $('.deleteSavedArticleButton').on("click", function(event) {
        ...
        $.ajax("/api/delete/article/" + articleId, {
            ...
            let newText = $('<div>');
            newText.text("Article has been deleted from your Saved Articles");
    ...
    $('.addNoteButton').on("click", function(event) {
        ...
        $.ajax("/api/notes/" + articleId, {
            ...
            $('.noteModalBody').append("<h4>" + result.headline + "</h4>");
            ...
            for (let i = 0; i < result.note.length; i ++) {
                ...
                newCard.append("<button class='btn btn-danger deleteNoteButton' data-id=" + i + ">Delete</button>");
